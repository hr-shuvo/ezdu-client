            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                {modules.map(({ title, subTitle, totalCourse, isPopular, emoji }, idx) => (
                    ...
                    >
                        ...
                        <div className="mb-3 text-sky-600 text-5xl">{emoji || '📘'}</div>


                        <h3 className="text-xl font-semibold mb-1">{title}</h3>
                        <p className="text-sky-700 text-sm mb-2 ">{subTitle}</p>
                        <p className="text-sky-900 font-medium">{totalCourse} Courses</p>
                    </div>
                ))}
            </div>
        </div>
